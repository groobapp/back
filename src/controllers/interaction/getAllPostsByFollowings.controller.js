...
export const getAllPostsByFollowings = async (req, res, next) => {
    try {
        ...
        const myPosts = myUser.publications.map((id) => id)
        ...
        const allMyIds = myUser.followings.map((id) => id)
        ...
        const uniqueIds = [...new Set(myPosts.map(post => post._id.toString()))];
        const noDuplicates = uniqueIds.map(id => myPosts.find(post => post._id.toString() === id));
        const data = noDuplicates.sort((a, b) => b.createdAt - a.createdAt);
        res.status(200).json(data)
